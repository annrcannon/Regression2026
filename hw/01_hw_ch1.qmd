...
-   Go to our RStudio Server at <https://csgateway.cornellcollege.edu:8787/>
...
-   Finding the instructions on our website: [{{< var course.url >}}]({{< var course.url >}}).
...
1.  (C1) **Applications that do not violate assumptions for inference in LLSR**. Identify the response and explanatory variable(s) for each problem. Write the LLSR assumptions for inference in the context of each study.
...
2.  (C2) **Applications that do violate assumptions for inference in LLSR**. All of the examples in this section have at least one violation of the LLSR assumptions for inference. Begin by identifying the response and explanatory variables. Then, identify which model assumption(s) are violated.
...
4.  (G3) **Housing prices and log transformations.** The dataset `kingCountyHouses.csv` contains data on over 20,000 houses sold in King County, Washington ([Kaggle, 2018a](https://www.kaggle.com/datasets/harlfoxem/housesalesprediction)). The dataset includes the following variables:
    ...
    k.  Fit Model 4, with `logprice` and `logsqft` as the response and explanatory variables, respectively. Write out the regression line equation.
...
5.  (O1) **The Bechdel Test.** In April, 2014, the website FiveThirtyEight published the article, "The Dollar-And-Cents Case Against Hollywood's Exlusion of Women" [Hickey, 2014].  There, they analyze returns on investment for 1,615 films released between 1990 and 2013 based on the Bechdel test.  The test, developed by cartoonist Alison Bechdel, measures gender bias in films by checking if a film meets three criteria:
