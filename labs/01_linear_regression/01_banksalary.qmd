...
1) Log into the course RStudio Server. <https://csgateway.cornellcollege.edu:8787/> Make sure to close out of any projects on the top right corner. 
...
4) Copy lab folder called "01_linear_regression" located at *Home -> STA363_inst_files-> labs*. If you check the box next to the folder name, then click the small gear icon you can "copy to" and put a copy of the folder in your newly created folder. 
5) Now, click *Project:(None)* in the top right corner of your screen and choose *New Project* and "Don't Save". 
6) Choose *Existing Directory* and navigate to the directory you just copied.
7) You can use *01_banksalary.qmd* for exploring with code.
8) Place your responses in the file *01_banksalary_fillable.qmd*.

## Introduction

**Example: Sex discrimination in bank salaries.** In the 1970’s, Harris Trust was sued for sex discrimination in the salaries it paid its employees. One approach to addressing this issue was to examine the starting salaries of all skilled, entry-level clerical workers between 1965 and 1975. The data is saved under banksalary.csv, in the data folder of the project. You will find relevant code at the end of these instructions. 
...
11. Does **model2** from question 10 meet all linear regression assumptions? List each assumption and how you decided if it was met or not.
...
13. You should have noticed that the term for age was not significant in the model with four predictors (*senior*, *age*, *educ*, and *exper*). What does this imply about age and about future modeling steps?

14. The relationship between experience and beginning salary exhibits some curvature. How might it be interpreted in this context? Determine whether a quadratic term in experience improves a model without curvature (see code for **model5**).

15. Consider a model with the predictors *senior*, *educ*, *exper*, and *male*. What conclusions can be drawn about sex discrimination at Harris Trust? Do these conclusions have to be qualified at all, or are they pretty clear cut? Interpret a 95% confidence interval for the male indicator variable in context to help with your response (see code for **model6**).
...
18. **Model6a** predicts *log(bsal)* with predictors *senior*, *educ*, *exper*, and *male*. Regardless of your answer to (17), provide an interpretation for the coefficient for the male coefficient in **model6a** after logging starting salary.
...
#library(ggplot2)
#library(gridExtra)
bank <- read_csv("data/banksalary.csv")
```
