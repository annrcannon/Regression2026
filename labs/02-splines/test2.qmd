...
1.  Explore the `wage` dataset. What are the variables? How many observations are there? Is there any missing data? If so, how much? For simplicity in this lab, we are going to perform a *complete case analysis*. Create a new dataframe that is complete (only based on education, not `maritl` since we are not going to use that variable in this lab).

2.  For this lab, we are interested in predicting the variable `wage` from `age`, `health_ins`, `jobclass`, `education`, and `race`. Complete a brief univariate and bivariate EDA for each of these variables and the response.

3.  Why is the default to return orthogonal vectors (perpendicular vectors)? What advantage does this have over the regular vectors for regression?
