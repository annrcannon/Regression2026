...
```{r}
beer <- read_csv("data/beer_wine_data.csv")
head(beer)
```
...
ggplot(data = beer, aes(x = Carbs, y = Calories)) + 
  geom_point() + 
  geom_smooth(method = "lm", se = FALSE) + 
  labs(x = "Carbs", y = "Calories", 
       title = "Calories vs. carbs") 
```
...
```{r echo = F}
model1 <- lm(Calories ~ Carbs, data=beer)
...
$$\widehat{Calories} = 109.815 + 4.351 ~ Carbs$$
...
1. Write out the interpretations for `Carbs`. 
...
## Interpreting quadratic effects {.smaller}
