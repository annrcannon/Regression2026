...
- Review linear regression (normal response variables)
...
- [{{< var college.rstudio >}}]({{< var college.rstudio >}})
- All of you have accounts from last spring.  You should use that password.  If you don't remember your password, please see Ajit Chavan in the CS department to get it changed.
...
- **Office hours** to meet with your instructor in {{< var instructor.office >}}
  - Typically {{< var instructor.officehrs >}}  Note that if my door is open, you are welcome to come ask questions!
...
### Create new variable
 ...
 - Create a new variable: $c\_year = year - 1979$
...
$$\widehat{extent} = 7.625 - 0.076 ~ c\_year$$
...
Questions:
 ...
 - What is the interpretation of the y-intercept now? 
...
```{r echo = F}
si |>
  mutate(yearsq = c_year*c_year) |>
  mutate(yearcu = yearsq*c_year) -> si

model3 <- lm(extent ~ year+ yearsq + yearcu, data=si)
