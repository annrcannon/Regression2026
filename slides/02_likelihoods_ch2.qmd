...
Question: On Monday we reviewed simple linear regression.  How did we determine the equation for the best model for Y given a particular explanatory variable X?

 . . .

Answer: We used the sum of the squared residuals. 

 . . .

But this only works for cases where Y has a Normal distribution.

## Example:

Space Shuttle Challenger:  The space shuttle Challenger mission, set to fly on January 28, 1986, was highlighted across the country as being a mission for students.  For the first time, a high school teacher was chosen for astronaut training and planned to deliver lessons to students across the country from space.

 . . .
 
There was controversy over launching the shuttle that day because the forecasted temperature was $26^\circ$F.  The coldest previous launch was $53^\circ$F.  Engineers were concerned that a part called the O-ring would fail at lower temperatures.

## Predict failure from previous data? {.smaller}

Let's plot whether there were O-ring failures against the launch temperature for all shuttle flights before the fated Challenger flight.

```{r, echo=FALSE}
shuttle <- read.csv("data/Shuttle.CSV")

ggplot(data=shuttle, aes(x=Temp, y=ODamage)) +
  geom_point()
...
Does linear regression make sense? What does a residual mean here?

## New idea

We need a new way to define "best" for a model where least squares doesn't work.  

 - In this chapter we introduce a method called maximum likelihood
 
 - Maximum likelihood uses the idea (and function called) likelihood
 
 - Maximum likelihood plays the same role that least squares played in regression with Normal error terms.
...
- Unlike Ordinary Least Squares (OLS), the likelihood does not require the responses to be independent, identically distributed, and normal (iidN)

- Likelihoods are **not** the same as probability functions

  - **Probability function:** Fixed parameter value(s). Input possible outcomes $\Rightarrow$ probability of seeing these outcomes given the parameter value(s)
  

  
  - **Likelihood:** Fixed data.  Input possible parameter values $\Rightarrow$ probability of seeing the fixed data for that particular parameter value
...
- 90 total fouls overall (3 fouls in each of 30 games)
...
Since calculus is not a pre-req, we will forgo this quest. See STA 347/348.
...
Original edits by Tyler George
