...
subtitle: "BMLR Chapters 3 and 5"
format: 
  revealjs:
...
- All information in this chapter is covered in much greater depth in STA 347.
...
- We use $\E(Y)=\mu$ to represent the mean of the distribution
- We use $\SD(Y)=\sigma$ to represent the standard deviation of the distribution
...
- If the coin lands tails, then we record $Y=0$.
- A series of these events is a  __Bernoulli process__ \index{Bernoulli process} if these events consist of 
  - independent trials 
  - each trial takes on one of two values (e.g., 0 or 1). 
  - the probability of success is identical for each trial.
- Typically we refer to the value 0 as a failure and the value of 1 as a success.
...
- If $Y \sim \textrm{Binary}(p)$ \left[or $Y \sim \textrm{Ber}(p)$\right], then $Y$ has mean $\E(Y) = p$ and standard deviation $\SD(Y) = \sqrt{p(1-p)}$.
...
- If we consider a case where $p = 0.25$ and $n = 4$, then here $P(Y=0)$ represents the probability of no successes in 4 trials (no heads in 4 flips)
...
- There are several ways to get exactly 1 success in 4 trials.  What is the probability for one way of these ways?
- To find the total probability of getting exactly 1 success among the 4 trials what else do we need to count? 

- There are $\binom{4}{1} = \frac{4!}{1!(4-1)!}=4$ ways to have exactly 1 success in 4 trials, 
...
- $E(\cdot)$ is the mean of a random variable 
...
- Note: If r is an integer, then $\Gamma(r)=(r-1)!$ (There is more to it)
...
- Exponential distributions are in fact a special case of gamma distributions!
...
- These slides are based on content in [BMLR: Chapter 3 - Distribution Theory](https://bookdown.org/roback/bookdown-BeyondMLR/ch-distthry.html)
- Edited from slides by Tyler George
...
[Nelder and Wedderburn (1972)](https://sakai.duke.edu/access/content/group/ec265469-bdb3-47a7-beb0-317956b6f86f/GLM-paper.pdf) define a broad class of models called **generalized linear models** that generalizes multiple linear regression. GLMs are characterized by three components: 
...
- These slides are based on content in [BMLR: Chapter 5](https://bookdown.org/roback/bookdown-BeyondMLR/ch-glms.html)
