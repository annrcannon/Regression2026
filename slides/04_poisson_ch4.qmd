...
Each response variable is a count per unit of time or space.
...
- The distribution becomes more symmetric as $\lambda$ increases
  - If $\lambda$ is sufficiently large, it can be approximated using a normal distribution ([Click here](https://online.stat.psu.edu/stat414/Lesson28#normal-approximation-to-poisson) for an example.)
...
## Use GLM to find a model for $\lambda$? {.smaller}
...
1. Can the distribution of the response variable be written in a one-parameter exponential family form? $f(y;\lambda)=e^{[a(y)b(\lambda)+c(\lambda)+d(y)]}$



2. Does the support depend on the parameter $(\lambda)$?


- If the answer to 1 is yes and the answer to 2 is no, then we can use the canonical link and try modeling $b(\lambda)= \beta_0 + \beta_1 x_1 + \beta_2 x_2 + ...$

### Answers

1. $P(Y=y)=\frac{e^{-\lambda}\lambda^y}{y!}=e^{-\lambda}e^{ylog\lambda}e^{-\log(y!)}=e^{y\log\lambda-\lambda-\log(y!)}$ which is in the one-parameter exponential family form.

2. The support of the probabilty function is $y=0,1,2,...$ which does not depend on $\lambda$.

- This suggests that the model should be in the form $\log(\lambda)=\beta_0 + \beta_1 x_1 + \beta_2 x_2 + ...$
...
## Poisson vs. multiple linear regression {.smaller}
...
## **Which can help determine if Model 1 is a good fit?** {.smaller}
...
As with logistic regression, we can use a **drop-in-deviance test** to compare two models. To conduct the test 
...
The **deviance residuals** indicate how much the observed data deviates from the fitted model
...
$$\text{residual deviance} = \sum{\left(\text{deviance residual}\right)_i^2}$$
...
## Compare all three models {.smaller}
...
|(intercept)|-0.3843|0.181|-0.3843|0.2166|-0.3753|0.2076|
|age | 0.0704|0.0069|0.0704|0.0082|0.0699|0.0079|
|age2 | -0.0007|0.0001|-0.0007|0.0001|-0.0007|0.0001|
|locDavao|-0.0194|0.0538|-0.0194|0.0640|-0.0219|0.0625|
|locIllocos|0.0610|0.0527|0.0610|0.0626|0.0577|0.0615|
|locManila|0.0545|0.0472|0.0545|0.0561|0.0562|0.0551|
|locVisayas|0.1121|0.0417|0.1121|0.0497|0.1104|0.0487|
...
The data [`weekend-drinks.csv`](data/weekend-drinks.csv) contains information from a survey of 77 students in an introductory statistics course on a dry campus. 
...
- The ZIP model is a special case of a **latent variable model**
  - A type of **mixture model** where observations for one or more groups occur together but the group membership is unknown
