...
They were particularly interested in understanding the implementation of prevention practices at food establishments, given the workers' increased risk due to daily contact with customers. 
...
## Interpreting the results (1/2) {.smaller}

Odds and Odds Ratios

- The odds of a response variable are: $\text{Odds} = \frac{\text{Num yes}}{\text{Num no}}$
  - If $\text{Odds} > 1$ then yeses are more likely than noes.
  - If $\text{Odds} < 1$ then noes are more likely than yeses.
  - If $\text{Odds} = 1$ then the probability of yeses and noes is the same.
  
- The odds ratio compares the odds of two different groups.  Often these groups consist of the "treated" group and the "control" group.  Or, the group of interest and the reference group.
  - $\text{Odds ratio} = \frac{\text{Odds of interest group}}{\text{Odds of reference group}}=\frac{\text{Odds of treatment group}}{\text{Odds of control group}}$


## Interpreting the results (2/2)  {.smaller}
...
The deviance residuals also compare the actual and predicted counts, but do so using the theory of likelihoods.
...
$\Sigma d_i =$ deviance or residual deviance
...
rr_aug <- augment(rr_model, type.predict = "response", 
                        type.residuals = "deviance")

rr_aug |> slice(1:5) |> kable()
```
...
```{r echo = F}
ggplot(data = rr_aug, aes(x = .fitted, y = .resid)) + 
  geom_point() + 
...
## Fit an interaction term {.smaller}
...
rr_int_model <- glm(cbind(YesVotes, NumVotes-YesVotes) ~ distance + pctBlack +
                      distance*pctBlack, data=rr, family=binomial)
...
```{r, warning=F,message=F}
rr_int_model <- glm(cbind(YesVotes, NumVotes - YesVotes) ~ distance + pctBlack + distance*pctBlack, data = rr, family = binomial)
...
tidy(rr_simp_model) |> kable(digits=3)
```
...
## Adjusting for overdispersion (1/4) {.smaller} 


- We can adjust for overdispersion in the binomial regression model by using the dispersion parameter $$\hat{\phi} = \sum \frac{(\text{Pearson residuals})^2}{n-p}$$
...
## Adjusting for overdispersion (2/4) {.smaller}
...
## Adjusting for overdispersion (3/4) {.smaller}

:::{.panel-tabset}

### Quasibinomial output {.smaller}

```{r, echo=F}
rr_qua_model <- glm(cbind(YesVotes, NumVotes - YesVotes) ~ distance + pctBlack + distance*pctBlack, data = rr, family = quasibinomial)

tidy(rr_qua_model) |> kable(digits=3)
```
...
## Adjusting for overdispersion (4/4) {.smaller}

:::{.panel-tabset}

### Without interaction output {.smaller}

```{r, echo=F}
rr_qua2_model <- glm(cbind(YesVotes, NumVotes - YesVotes) ~ distance + pctBlack, data = rr, family = quasibinomial)

tidy(rr_qua2_model) |> kable(digits=3)
...
```{r, echo=F}
rr_qua3_model <- glm(cbind(YesVotes, NumVotes - YesVotes) ~ distance, data = rr, family = quasibinomial)

tidy(rr_qua3_model) |> kable(digits=3)
