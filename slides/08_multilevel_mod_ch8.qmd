...
The data [`musicdata.csv`](data/musicdata.csv) come from the Sadler and Miller (2010) study of the emotional state of musicians before performances. The dataset contains information collected from 37 undergraduate music majors who completed the Positive Affect Negative Affect Schedule (PANAS), an instrument which produces a measure of anxiety (negative affect) and a measure of happiness (positive affect). This analysis will focus on negative affect as a measure of performance anxiety.
...
We'll start with the Level One model to understand the association between performance type and performance anxiety for the $i^{th}$ musician. The index $j$ references the specific performance.
...
`r emo::ji("warning")`  Does not share strength effectively across individuals (see histogram of $R^2$ values in example file) 
...
- The **fixed effects** to estimate are $\alpha_0$, $\alpha_1$, $\beta_0$, and $\beta_1$.
- The **error terms** are $u_i$, $v_i$, and $\epsilon_{ij}$.
   - $u_i$ and $v_i$ are associated with musician random effect
   - $\epsilon_{ij}$ is what's left unexplained
- We no longer need to estimate $a_i$ and $b_i$.
...
- For the Level Two models, the errors are 
  - $u_i$: deviation of musician $i$ from the mean performance anxiety before solos and small ensembles after accounting for the instrument 
     - musician-to-musician differences are in the intercepts
  - $v_i$: deviance of musician $i$ from the mean difference in performance anxiety between large ensembles and other performance types after accounting for instrument
     - musician-to-musician differences are in the slopes
...
[Question: **Describe what we learn about the association between the slopes and intercepts based on this plot.** ]{style="font-size:0.8em;"}
...
Fit the multilevel model using the `lmer` function from the **lme4** package.  Display results using the `tidy()` function from the **broom.mixed** package.
...
$$\begin{aligned} Y_{ij}&= \space \alpha_0 + \alpha_1 ~ Orchestra_i+ u_i + \beta_0LargeEnsemble  \\ 
&  \space\space\space  + \beta_1Orchestra_iLargeEnsemble+v_iLargeEnsemble+\epsilon_{ij}\\
&=\alpha_0 + \alpha_1 ~ Orchestra_i + \beta_0LargeEnsemble+\beta_1Orchestra_iLargeEnsemble \\
& \space\space\space +u_i + v_iLargeEnsemble+\epsilon_{ij}
\end{aligned}$$

- All variables with greek letters are fixed effects and all variables with $u$, $v$ are random effects. 

. . .


lmer(na ~ Orchestra + LargeEnsemble + Orchestra * LargeEnsemble + (LargeEnsemble|id),data = music)
...
$$\begin{aligned}na_{ij} &= 15.930 + 1.693 ~ Orchestra_i + 0.911 ~ LargeEnsemble_{ij} \\ 
&- 1.424 ~ Orchestra_i:LargeEnsemble_{ij}
\end{aligned}$$
...
$$\epsilon_{ij} \sim N \left( 0,4.670^2 \right)$$
