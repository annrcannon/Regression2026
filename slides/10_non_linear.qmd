...
format: 
  revealjs:
    output-file: "10_non_linear_ch7.html"
    slide-number: true
  html:
    output-file: "10_non_linear_ch7_o.html"
...
::: question
Using the information below, write out the equation to predicte change
in population from a change in age from the 25th percentile (24.5) to the
...
Below is a linear regression model fit to include the 3 bases you just created with 2 knots: 35 and 65. Use the information here to hand draw the relationship between $x$ and $y$.
...
## How does it fit the data? {.small}
...
ggplot() +
  geom_point(data=newdat, aes(x = b1, y = p)) + 
  geom_point(data=d, aes(x=b1, y=y), color="red") +
  labs(x = "X",
       y = expression(hat(y)))
```
...
## Cubic Splines with data added to plot {.small}
